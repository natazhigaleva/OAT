...
class Resource:
    def __init__(self, type_resource, quantity_of_available):
        self.type_resource = type_resource
        self.quantity_of_available = quantity_of_available
        self.quantity_using = 0

    def add(self, quantity):
        self.quantity_of_available += quantity

    def allot(self, quantity):
        if quantity <= self.quantity_of_available:
            self.quantity_of_available -= quantity
            self.quantity_using += quantity
        else:
            print("Недостаточно ресурсов.")

    def vacate(self, quantity):
        if quantity <= self.quantity_using:
            self.quantity_of_available += quantity
            self.quantity_using -= quantity
        else:
            print("Невозможно освободить больше ресурсов, чем используется.")

    def show(self):
        print(f"Тип ресурса: {self.type_resource}")
        print(f"Доступное количество: {self.quantity_of_available}")
        print(f"Используемое количество: {self.quantity_using}")


# Пример использования
resource_1 = Resource("Рабочие", 10)
resource_2 = Resource("Материалы", 20)
